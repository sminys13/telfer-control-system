...
/**
 * @brief CRC32 (полином 0xEDB88320) БЕЗ таблицы.
 *        Экономит ~1024 байта SRAM (таблица 256*4), чуть медленнее, но для EEPROM это нормально.
 */
...
/**
 * @brief Расчет контрольной суммы CRC32
 * @param data Указатель на данные (NULL для расчета по всей EEPROM от 0 до ADDR_CRC-1)
 * @param size Размер данных (используется только если data != NULL)
 * @return CRC32
 */
uint32_t calculateCRC(const void *data, uint16_t size)
{
    ...
    {
        // CRC по всей EEPROM до поля CRC (ADDR_CRC) — как у тебя было раньше
        for (uint16_t i = 0; i < ADDR_CRC; i++)
        {
            crc = crc32Update(crc, EEPROM.read(i));
        }
    }
