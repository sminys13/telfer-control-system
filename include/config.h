...
// Пины управления RS-485 (Двигателя)
#define RS485_RE_DE_PIN    6  // Управление направлением (RE/DE)
#define RS485_TX_ENABLE    7  // Разрешение передачи (опционально)
#define RS485_TX_PIN      18  // TX1 (Serial1)
#define RS485_RX_PIN      19  // RX1

// Лазерные дальномеры
#define LASER1_TX_PIN     16  // TX2 (Serial2)
#define LASER1_RX_PIN     17  // RX2
#define LASER2_TX_PIN     14  // TX3 (Serial3)
#define LASER2_RX_PIN     15  // RX3
...
#define LOOP_INTERVAL            10     // Целевое время цикла 10 мс (100 Гц)
#define DISPLAY_UPDATE           50     // Обновление дисплея каждые 50 мс (20 FPS)
#define ENCODER_DEBOUNCE         50     // Время антидребезга энкодера

// Лимиты системы
// #define MAX_PROGRAMS 1              // Максимальное количество программ
#define MAX_ZONES                10    // Максимальное количество зон в программе
#define MAX_ZONE_NAME             8    // Длина имени зоны
#define MAX_PROGRAM_NAME         10    // Длина имени программы
...
// ========== СТРУКТУРЫ ДАННЫХ ==========
#pragma pack(push, 1)  // Выравнивание 1 байт для экономии памяти
/**
 * @brief Параметры зоны обработки
 */
typedef struct
{
    char name[MAX_ZONE_NAME];        // Название зоны
    int16_t position;                // Горизонтальная позиция (мм)
    int16_t height;                  // Целевая высота (мм)
    ...
    uint8_t speed;                   // Скорость движения к зоне (%)
...
typedef struct
{
    int32_t pos1;             // Позиция тельфера 1 (мм)
    int32_t pos2;             // Позиция тельфера 2 (мм)
    int32_t height1;          // Высота груза 1 (мм)
    int32_t height2;          // Высота груза 2 (мм)
    ...
    bool emergency;           // Аварийная остановка активна
    bool sensorsOk;           // Данные датчиков валидны
} SystemState;
...
// ========== МАКРОС ДЛЯ СТРОК В FLASH ==========
#define PSTR_TO_BUFFER(pstr, buf) strcpy_P(buf, (const char*)pstr)
